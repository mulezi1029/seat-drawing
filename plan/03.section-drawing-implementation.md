...
```typescript
function snapToGrid(point: Point, gridSize: number, scale: number): SnapResult | null {
  const snapped = {
    x: Math.round(point.x / gridSize) * gridSize,
    y: Math.round(point.y / gridSize) * gridSize,
  };
  // 10px 屏幕像素容差（转换为世界坐标容差）
  const gridTolerance = 10 / scale;
  if (getDistance(point, snapped) < gridTolerance) {
...
**注意：** 网格吸附同样需要考虑缩放比例，使用 `10 / scale` 作为容差，确保在任意缩放级别下用户体验一致。

### 4.2 顶点吸附

```typescript
// 15px 屏幕像素容差（转换为世界坐标容差）
const vertexTolerance = 15 / scale;
for (const vertex of vertices) {
  if (getDistance(point, vertex) < vertexTolerance) {
    return { point: vertex, type: 'vertex', source: vertex };
  }
}
```

**注意：** 容差需要根据缩放比例调整，确保在任意缩放级别下，用户在屏幕上的感知距离保持一致。
...
所有像素容差均为屏幕像素，通过 `tolerance / scale` 转换为世界坐标容差，确保在任意缩放级别下用户体验一致：

1. 顶点吸附（15px 屏幕像素容差，`15/scale`）
2. 网格吸附（10px 屏幕像素容差，`10/scale`）
3. 角度吸附（5° 容差，Shift 模式）
4. 对齐检测（5px 屏幕像素容差，`5/scale`，自动吸附 X/Y 坐标）
...
- [ ] 右键取消绘制
- [ ] 网格吸附（可选，10px 屏幕像素容差，缩放自适应）
- [ ] 顶点吸附（15px 屏幕像素容差，缩放自适应）
