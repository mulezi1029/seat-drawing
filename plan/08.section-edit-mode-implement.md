# 区域编辑模式实现文档

> 基于 `plan/09.section-edit-features.md` 功能点清单编写
> 本文档按实现阶段组织，用于指导开发

---

## 概述

区域编辑模式允许用户"进入"某个已绘制区域，在新的画布层中进行座位绘制。采用**可视化校准**方式—：通过交互式缩放和座位尺寸设置来完成校准。

---

## 数据结构定义

### 1. Section 扩展

```typescript
interface Section {
  id: string;
  name: string;
  points: Point[];
  color: string;
  seats: Seat[];
  opacity: number;
  /** 校准设置持久化 */
  calibrationData?: {
    canvasScale: number;
    anchorScale: number;
    seatVisual: {
      width: number;
      height: number;
      gapX: number;
      gapY: number;
    };
    isCalibrated: boolean;
  };
}
```

### 2. 座位数据结构

```typescript
interface Seat {
  id: string;
  /** 世界坐标系中的位置 */
  x: number;
  y: number;
  /** 行号，如 "A" */
  row: string;
  /** 座位号 */
  number: number;
  /** 座位角度（相对于正北，顺时针） */
  angle: number;
  /** 座位类型 */
  type: 'normal' | 'vip' | 'accessible' | 'empty';
  /** 视觉尺寸比例 */
  visualScale: number;
}
```

### 3. 区域编辑状态

```typescript
interface SectionEditState {
  /** 是否处于区域编辑模式 */
  isActive: boolean;
  /** 正在编辑的区域 ID */
  sectionId: string | null;
  /** 当前阶段 */
  phase: 'calibrating' | 'editing';
  /** 可视化校准状态 */
  calibration: {
    canvasScale: number;
    anchorScale: number;
    seatVisual: {
      width: number;
      height: number;
      gapX: number;
      gapY: number;
    };
    isCalibrated: boolean;
  };
  /** 当前编辑工具 */
  activeTool: 'select' | 'seat-row' | 'seat-matrix' | 'seat-arc';
  /** 编辑中的座位 */
  seats: Seat[];
  /** 选中的座位ID列表 */
  selectedSeatIds: string[];
  /** 是否有未保存的更改 */
  hasUnsavedChanges: boolean;
}
```

---

## 第一阶段：基础框架与进入/退出

**目标**：建立区域编辑模式的基础框架，实现进入和退出功能。

### 1.1 类型扩展 (10.1)

| 功能点 | 文件 | 说明 |
|--------|------|------|
| Section.calibrationData | `types/index.ts` | 扩展 Section 类型 |

**实现**：
```typescript
// types/index.ts
export interface Section {
  // ... 现有字段
  calibrationData?: CalibrationData;
}

export interface CalibrationData {
  canvasScale: number;
  anchorScale: number;
  seatVisual: SeatVisualParams;
  isCalibrated: boolean;
}
```

### 1.2 状态管理 Hook (10.2)

| 功能点 | 文件 | 说明 |
|--------|------|------|
| useSectionEdit | `hooks/useSectionEdit.ts` | 状态管理 Hook |

**实现**：
```typescript
// hooks/useSectionEdit.ts
export function useSectionEdit() {
  const [state, setState] = useState<SectionEditState>({
    isActive: false,
    sectionId: null,
    phase: 'calibrating',
    calibration: {
      canvasScale: 1,
      anchorScale: 1,
      seatVisual: { width: 20, height: 20, gapX: 5, gapY: 5 },
      isCalibrated: false,
    },
    activeTool: 'select',
    seats: [],
    selectedSeatIds: [],
    hasUnsavedChanges: false,
  });

  const enterEditMode = useCallback((section: Section) => {
    // 检查是否已有校准数据
    const hasCalibration = section.calibrationData?.isCalibrated;
    // ... 初始化状态
  }, []);

  const exitEditMode = useCallback(() => {
    // ... 退出逻辑
  }, []);

  return { state, setState, enterEditMode, exitEditMode };
}
```

### 1.3 主容器组件 (10.4)

| 功能点 | 文件 | 说明 |
|--------|------|------|
| SectionEditModal | `components/section-edit/SectionEditModal.tsx` | 主容器 |

**实现要点**：
- 全屏模态框覆盖
- 根据 phase 显示不同布局
- 传递状态和方法给子组件

### 1.4 进入方式 (1.1, 1.3, 1.4)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 双击区域进入 | ⬜ | 主画布区域双击事件 |
| 工具栏"进入编辑"按钮 | ⬜ | 选中区域时显示 |
| Enter 快捷键进入 | ⬜ | 需选中单个区域 |

**实现**：
```typescript
// Canvas.tsx - 双击处理
const handleSectionDoubleClick = (section: Section) => {
  enterEditMode(section);
};

// 选中区域时的工具栏按钮
{selectedSection && (
  <button onClick={() => enterEditMode(selectedSection)}>
    进入编辑
  </button>
)}

// 快捷键处理
useEffect(() => {
  const handleKeyDown = (e: KeyboardEvent) => {
    if (e.key === 'Enter' && selectedSections.length === 1) {
      enterEditMode(selectedSections[0]);
    }
  };
  // ...
}, [selectedSections]);
```

### 1.5 进入前检查 (1.5, 1.6)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 单选检查 | ⬜ | 多选时提示"请只选择一个区域" |
| 多边形有效性检查 | ⬜ | points.length >= 3 |

**实现**：
```typescript
const enterEditMode = (section: Section) => {
  if (selectedSections.length !== 1) {
    alert('请只选择一个区域进行编辑');
    return;
  }
  if (section.points.length < 3) {
    alert('该区域没有有效的多边形边界');
    return;
  }
  // ... 进入编辑模式
};
```

### 1.6 退出与保存 (1.7, 1.9)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 点击返回按钮退出 | ⬜ | 顶部面板返回按钮 |
| 保存后退出 | ⬜ | 保存座位+校准数据 |

**实现**：
```typescript
const handleSave = () => {
  // 转换坐标
  const worldSeats = seats.map(seat => ({
    ...seat,
    x: seat.x + bounds.minX,
    y: seat.y + bounds.minY,
  }));

  // 保存到 Section
  onSaveSection({
    ...section,
    seats: worldSeats,
    calibrationData: calibration,
  });

  exitEditMode();
};
```

---

## 第二阶段：校准模式

**目标**：实现可视化校准界面，让用户调整画布缩放和座位参数。

### 2.1 校准画布 (2.1.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 显示完整底图 SVG 背景 | ⬜ | opacity: 0.7 |
| 区域内部底图高亮显示 | ⬜ | clipPath 裁剪, opacity: 0.8 |
| 区域外部半透明遮罩 | ⬜ | mask 实现, opacity: 0.5 |
| 区域边框（虚线） | ⬜ | strokeDasharray |
| 画布缩放 50% - 1000% | ⬜ | 滑块+滚轮控制 |
| 画布平移（空格+拖拽） | ⬜ | |
| 自动居中定位区域 | ⬜ | ResizeObserver |

**实现**：`components/section-edit/SectionEditCanvas.tsx`

```tsx
<svg width={WORLD_SIZE} height={WORLD_SIZE}>
  <g transform={worldLayerTransform}>
    {/* 完整底图背景 */}
    {svgUrl && (
      <image
        href={svgUrl}
        x={svgX}
        y={svgY}
        width={svgWidth}
        height={svgHeight}
        opacity={0.7}
      />
    )}

    {/* 区域高亮（裁剪显示） */}
    <g clipPath="url(#section-clip)">
      {svgUrl && (
        <image
          href={svgUrl}
          x={svgX}
          y={svgY}
          width={svgWidth}
          height={svgHeight}
          opacity={0.8}
        />
      )}
    </g>

    {/* 区域外部遮罩 */}
    <mask id="section-mask">
      <rect x={0} y={0} width={WORLD_SIZE} height={WORLD_SIZE} fill="white" />
      <polygon points={pointsString} fill="black" />
    </mask>
    <rect
      x={0} y={0} width={WORLD_SIZE} height={WORLD_SIZE}
      fill="black"
      opacity={0.5}
      mask="url(#section-mask)"
    />

    {/* 区域边框 */}
    <polygon
      points={pointsString}
      fill="none"
      stroke={section.color}
      strokeWidth={2 / canvasScale}
      strokeDasharray="5,5"
    />
  </g>
</svg>
```

### 2.2 校准面板 (2.2.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 画布缩放滑块（50%-1000%） | ⬜ | |
| 预览矩阵切换按钮 | ⬜ | 2×2, 3×3, 4×4, 5×5, 实际 |
| 预计容量计算显示 | ⬜ | 根据参数计算 |
| 重置按钮 | ⬜ | R键快捷键 |

**实现**：`components/section-edit/CalibrationPanel.tsx`

```tsx
<div className="calibration-panel">
  {/* 画布缩放 */}
  <div className="control-group">
    <label>画布缩放</label>
    <Slider
      min={0.5}
      max={10}
      step={0.1}
      value={canvasScale}
      onChange={setCanvasScale}
    />
    <span>{Math.round(canvasScale * 100)}%</span>
  </div>

  {/* 预览矩阵 */}
  <div className="preview-options">
    {['2×2', '3×3', '4×4', '5×5', '实际'].map(size => (
      <button key={size} onClick={() => setPreviewSize(size)}>
        {size}
      </button>
    ))}
  </div>

  {/* 预计容量 */}
  <div className="capacity-estimate">
    预计容量: {estimatedRows} 排 × {estimatedCols} 列 = {estimatedCapacity} 座位
  </div>

  {/* 操作按钮 */}
  <div className="actions">
    <button onClick={resetCalibration}>重置 (R)</button>
    <button onClick={completeCalibration}>开始绘制座位</button>
  </div>
</div>
```

### 2.3 预览座位 (10.7)

| 功能点 | 文件 | 说明 |
|--------|------|------|
| SeatPreview | `components/section-edit/SeatPreview.tsx` | 预览座位矩阵 |

**实现要点**：
- 根据预览矩阵大小生成座位
- 使用校准的座位尺寸和间距
- 显示在画布中心

---

## 第三阶段：编辑模式界面

**目标**：实现与主世界一致的完整界面结构。

### 3.1 顶部面板适配 (3.1.x, 10.9)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 返回按钮 | ⬜ | 带返回图标 |
| 区域名称显示 | ⬜ | 格式："区域 A - 座位编辑" |
| 保存按钮 | ⬜ | 保存座位数据 |
| 帮助按钮 | ⬜ | 显示快捷键说明 |
| PanelTop mode="section-edit" 支持 | ⬜ | |

**实现**：
```tsx
// PanelTop.tsx
interface PanelTopProps {
  mode?: 'main' | 'section-edit';
  sectionName?: string;
  onBack?: () => void;
  onSave?: () => void;
}

// 编辑模式 UI
<div className="panel-top">
  <button onClick={onBack}>
    <ArrowLeftIcon /> 返回
  </button>
  <span>{sectionName} - 座位编辑</span>
  <div className="actions">
    <button onClick={onSave}>保存</button>
    <button>?</button>
  </div>
</div>
```

### 3.2 左侧面板适配 (3.2.x, 10.10)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 选择工具（V） | ⬜ | |
| 单排座位工具（1） | ⬜ | |
| 矩阵座位工具（2） | ⬜ | |
| 弧线排列工具（3） | ⬜ | |
| 工具激活状态显示 | ⬜ | |
| 工具快捷键提示 | ⬜ | 标签显示 (V), (1), (2), (3) |

**实现**：
```tsx
// PanelLeft.tsx - 编辑模式工具
const sectionEditTools = [
  { id: 'select', icon: MousePointer, label: '选择', shortcut: 'V' },
  { id: 'seat-row', icon: Plus, label: '单排座位', shortcut: '1' },
  { id: 'seat-matrix', icon: Grid3x3, label: '矩阵座位', shortcut: '2' },
  { id: 'seat-arc', icon: Circle, label: '弧线排列', shortcut: '3' },
];
```

### 3.3 右侧面板适配 (3.3.x, 10.11)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 座位属性区域 | ⬜ | |
| 行号输入框 | ⬜ | |
| 座位号输入框 | ⬜ | |
| 座位类型下拉 | ⬜ | 正常/VIP/无障碍/空位 |
| 座位角度输入 | ⬜ | 0-360° |
| 选中座位信息展示 | ⬜ | 未选中时显示提示 |

**实现**：
```tsx
// PanelRight.tsx - 编辑模式内容
{mode === 'section-edit' && selectedSeat && (
  <div className="seat-properties">
    <h3>座位属性</h3>
    <div className="form-group">
      <label>行号</label>
      <input
        value={selectedSeat.row}
        onChange={e => updateSeat({ row: e.target.value })}
      />
    </div>
    <div className="form-group">
      <label>座位号</label>
      <input
        type="number"
        value={selectedSeat.number}
        onChange={e => updateSeat({ number: +e.target.value })}
      />
    </div>
    <div className="form-group">
      <label>类型</label>
      <select
        value={selectedSeat.type}
        onChange={e => updateSeat({ type: e.target.value })}
      >
        <option value="normal">正常</option>
        <option value="vip">VIP</option>
        <option value="accessible">无障碍</option>
        <option value="empty">空位</option>
      </select>
    </div>
    <div className="form-group">
      <label>角度</label>
      <input
        type="number"
        min={0}
        max={360}
        value={selectedSeat.angle}
        onChange={e => updateSeat({ angle: +e.target.value })}
      />
    </div>
  </div>
)}
```

### 3.4 悬浮控件 (3.4.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 摇杆控制（上/下/左/右） | ⬜ | |
| 缩放控制（+/-） | ⬜ | |
| 重置中心按钮 | ⬜ | |
| 状态栏显示 | ⬜ | 坐标/排数/座位数 |

**实现**：复用现有的 FloatingControls 组件

---

## 第四阶段：座位绘制功能

**目标**：实现座位绘制、选择和编辑功能。

### 4.1 绘制工具 (4.1.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 单排座位放置 | ⬜ | 点击画布放置 |
| 矩阵拖拽绘制 | ⬜ | 拖拽确定行列 |
| 矩阵行数计算 | ⬜ | 根据拖拽距离 |
| 矩阵列数计算 | ⬜ | 根据拖拽距离 |
| 使用校准默认间距 | ⬜ | 矩阵绘制时 |
| 实时预览矩阵范围 | ⬜ | 拖拽时显示虚线框 |

**实现要点**：
```typescript
// 矩阵绘制逻辑
const handleMouseDown = (e: MouseEvent) => {
  setDragStart({ x: e.clientX, y: e.clientY });
  setIsDragging(true);
};

const handleMouseMove = (e: MouseEvent) => {
  if (!isDragging) return;

  const dx = e.clientX - dragStart.x;
  const dy = e.clientY - dragStart.y;

  // 根据距离和间距计算行列数
  const cols = Math.max(1, Math.floor(dx / (seatVisual.width + seatVisual.gapX)));
  const rows = Math.max(1, Math.floor(dy / (seatVisual.height + seatVisual.gapY)));

  setPreviewMatrix({ rows, cols });
};

const handleMouseUp = () => {
  if (isDragging && previewMatrix) {
    generateSeats(previewMatrix.rows, previewMatrix.cols);
  }
  setIsDragging(false);
  setPreviewMatrix(null);
};
```

### 4.2 座位选择 (4.2.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 单击选择单个座位 | ⬜ | |
| 框选多个座位 | ⬜ | |
| Ctrl+单击多选 | ⬜ | |
| 选择框实时显示 | ⬜ | |
| 选中座位高亮 | ⬜ | 边框或背景色变化 |

**实现要点**：
```typescript
// 选择逻辑
const handleSeatClick = (seatId: string, e: MouseEvent) => {
  if (e.ctrlKey || e.metaKey) {
    // Ctrl+单击：切换选择
    toggleSeatSelection(seatId);
  } else {
    // 单击：单选
    selectSingleSeat(seatId);
  }
};

// 框选逻辑
const handleSelectionBox = (start: Point, end: Point) => {
  const selectedIds = seats
    .filter(seat => isPointInBox({ x: seat.x, y: seat.y }, start, end))
    .map(seat => seat.id);
  setSelectedSeatIds(selectedIds);
};
```

### 4.3 座位编辑 (4.3.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 拖拽移动座位 | ⬜ | 实时预览 |
| 批量移动座位 | ⬜ | 多选时 |
| Delete 删除座位 | ⬜ | |
| 批量删除座位 | ⬜ | 多选时 |
| 修改行号 | ⬜ | 右侧面板 |
| 修改座位号 | ⬜ | 右侧面板 |
| 修改座位类型 | ⬜ | 右侧面板 |
| 修改座位角度 | ⬜ | 右侧面板 |

---

## 第五阶段：坐标转换与数据流

**目标**：实现世界坐标与局部坐标的转换，以及数据的保存和恢复。

### 5.1 坐标转换工具 (10.12)

| 功能点 | 文件 | 说明 |
|--------|------|------|
| worldToLocal | `utils/coordinate.ts` | 世界坐标转局部坐标 |
| localToWorld | `utils/coordinate.ts` | 局部坐标转世界坐标 |

**实现**：
```typescript
// utils/coordinate.ts
export function worldToLocal(
  worldPoint: Point,
  bounds: BoundingBox
): Point {
  return {
    x: worldPoint.x - bounds.minX,
    y: worldPoint.y - bounds.minY,
  };
}

export function localToWorld(
  localPoint: Point,
  bounds: BoundingBox
): Point {
  return {
    x: localPoint.x + bounds.minX,
    y: localPoint.y + bounds.minY,
  };
}
```

### 5.2 保存时坐标转换 (8.3)

| 功能点 | 说明 |
|--------|------|
| 保存时坐标转换 | 局部→世界 |
| 更新 Section.seats | |
| 保存 calibrationData | |

**实现**：
```typescript
const saveSeats = () => {
  const worldSeats = seats.map(seat => ({
    ...seat,
    x: seat.x + sectionBounds.minX,
    y: seat.y + sectionBounds.minY,
  }));

  onUpdateSection({
    ...section,
    seats: worldSeats,
    calibrationData: state.calibration,
  });
};
```

### 5.3 校准设置持久化 (5.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 完成校准时自动保存 | ⬜ | |
| 重新进入时自动恢复 | ⬜ | |
| 有校准数据时跳过校准 | ⬜ | 直接进入编辑模式 |

**实现**：
```typescript
const completeCalibration = () => {
  const calibrationData = {
    canvasScale: state.calibration.canvasScale,
    anchorScale: state.calibration.canvasScale,
    seatVisual: state.calibration.seatVisual,
    isCalibrated: true,
  };

  // 保存到校准数据
  onSaveCalibration(sectionId, calibrationData);

  // 切换到编辑阶段
  setState(prev => ({ ...prev, phase: 'editing' }));
};

// 进入时恢复
const enterEditMode = (section: Section) => {
  const hasCalibration = section.calibrationData?.isCalibrated;

  setState({
    isActive: true,
    sectionId: section.id,
    phase: hasCalibration ? 'editing' : 'calibrating',
    calibration: section.calibrationData ?? defaultCalibration,
    // ...
  });
};
```

---

## 第六阶段：画布交互

**目标**：实现与主画布一致的画布交互体验。

### 6.1 交互功能 (6.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 滚轮缩放 | ⬜ | Ctrl+滚轮 |
| 以鼠标位置为中心缩放 | ⬜ | |
| 空格+拖拽平移 | ⬜ | |
| 中键拖拽平移 | ⬜ | |
| 双击空白处重置缩放 | ⬜ | 重置到 anchorScale |
| 缩放范围限制（50%-1000%） | ⬜ | |

**实现要点**：参照主画布 Canvas.tsx 的实现，保持一致性。

```typescript
// 缩放处理（与主画布一致）
const handleWheel = useCallback((e: React.WheelEvent) => {
  if (e.ctrlKey || e.metaKey) {
    e.preventDefault();
    const delta = e.deltaY > 0 ? 0.9 : 1.1;
    const newScale = Math.max(0.5, Math.min(10, canvasScale * delta));

    // 以鼠标位置为中心缩放
    const rect = containerRef.current?.getBoundingClientRect();
    if (rect) {
      const mouseX = e.clientX - rect.left + containerRef.current!.scrollLeft;
      const mouseY = e.clientY - rect.top + containerRef.current!.scrollTop;
      setCanvasScale(newScale, mouseX, mouseY);
    }
  }
}, [canvasScale]);
```

---

## 第七阶段：快捷键

**目标**：实现编辑模式的快捷键支持。

### 7.1 快捷键映射 (7.x)

| 模式 | 按键 | 功能 | 状态 |
|------|------|------|------|
| 校准 | `Space` | 开始绘制座位 | ⬜ |
| 校准 | `R` | 重置所有参数 | ⬜ |
| 编辑 | `V` | 选择工具 | ⬜ |
| 编辑 | `1` | 单排工具 | ⬜ |
| 编辑 | `2` | 矩阵工具 | ⬜ |
| 编辑 | `3` | 弧线工具 | ⬜ |
| 编辑 | `Delete` | 删除选中座位 | ⬜ |
| 全局 | `Esc` | 退出编辑模式 | ⬜ |

---

## 第八阶段：主画布集成

**目标**：实现与主画布的集成。

### 8.1 主画布增强 (9.x)

| 功能点 | 状态 | 说明 |
|--------|------|------|
| 区域 hover 提示 | ⬜ | "双击编辑" |
| 选中区域时显示编辑入口 | ⬜ | 顶部浮动条 |
| 保存后主画布同步显示座位 | ⬜ | |
| 返回主画布保持视图状态 | ⬜ | 不重置缩放 |

---

## 文件清单

| 序号 | 文件路径 | 说明 | 状态 |
|------|----------|------|------|
| 10.1 | `app/src/types/index.ts` | 扩展 Section 类型 | ⬜ |
| 10.2 | `app/src/hooks/useSectionEdit.ts` | 状态管理 Hook | ⬜ |
| 10.3 | `app/src/components/section-edit/index.ts` | 组件导出 | ⬜ |
| 10.4 | `app/src/components/section-edit/SectionEditModal.tsx` | 主容器 | ⬜ |
| 10.5 | `app/src/components/section-edit/SectionEditCanvas.tsx` | 编辑画布 | ⬜ |
| 10.6 | `app/src/components/section-edit/CalibrationPanel.tsx` | 校准面板 | ⬜ |
| 10.7 | `app/src/components/section-edit/SeatPreview.tsx` | 预览座位 | ⬜ |
| 10.8 | `app/src/components/section-edit/SeatTools.tsx` | 座位工具栏 | ⬜ |
| 10.9 | `app/src/components/panels/PanelTop.tsx` | 添加 mode 支持 | ⬜ |
| 10.10 | `app/src/components/panels/PanelLeft.tsx` | 添加 mode 支持 | ⬜ |
| 10.11 | `app/src/components/panels/PanelRight.tsx` | 添加 mode 支持 | ⬜ |
| 10.12 | `app/src/utils/coordinate.ts` | 坐标转换函数 | ⬜ |

---

## 实现优先级

### 高优先级（基础功能）

1. 类型定义扩展（Section.calibrationData）
2. useSectionEdit Hook
3. SectionEditModal 主容器
4. 进入/退出编辑模式
5. 校准画布基础实现（背景、遮罩）
6. 校准面板 UI

### 中优先级（核心功能）

7. 面板适配（PanelTop/Left/Right mode 支持）
8. 座位绘制工具（单排、矩阵）
9. 座位选择功能
10. 坐标转换工具
11. 校准设置持久化

### 低优先级（优化功能）

12. 弧线排列工具
13. 快捷键支持
14. 撤销重做
15. 主画布集成增强

---

## 状态图例

| 符号 | 含义 |
|------|------|
| ⬜ | 未开始 |
| 🟡 | 进行中 |
| ✅ | 已完成 |
| ⏸️ | 暂停/阻塞 |

---

*文档创建日期: 2026-02-26*
*关联文档: plan/09.section-edit-features.md, plan/08.section-edit-mode.md*
