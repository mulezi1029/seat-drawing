...
基于实际使用反馈，对选择工具进行两项关键优化：

| 优化项 | 原有问题 | 优化后行为 |
|-------|---------|-----------|
| 选中框范围内拖拽 | 只有鼠标悬停在元素上才能拖拽 | 选中框范围内任意位置都可拖拽 |
| 元素选中范围 | 点击元素顶点构成的矩形框即可选中 | 使用实际多边形范围进行精确判断 |
...
**原有问题：**
- 选中元素后，只有当光标精确悬停在元素上时才显示 grab 光标并可拖拽
- 如果光标在选中框（边界框）内但在元素外，无法拖拽

**期望行为：**
- 光标在选中框（蓝色虚线框）范围内时，显示 grab 光标
- 点击选中框范围内任意位置都可以拖拽选中元素
- 提升操作便利性，不需要精确对准元素

#### C.2.2 实现方案

**核心思路：**
在 `handleMouseDown` 中增加判断：如果点击位置在选中元素的组合边界框内，即使没有精确点击到元素，也触发拖拽。
 ...
 * 计算多个边界框的组合边界框
 */
export function getCombinedBoundingBox(bboxes: BoundingBox[]): BoundingBox | null {
 ...
 * 检查点是否在选中元素的组合边界框内
 */
export function isPointInSelectedBbox(
  point: Point,
  selectedIds: Set<string>,
  sections: Section[]
): boolean {
  ...
  if (!combinedBbox) return false;
  return isPointInBoundingBox(point, combinedBbox);
}
```
...
// 鼠标是否在选中元素的边界框内（用于显示 grab 光标）
const isInSelectedBbox = mousePosition && selectedIds.size > 0
  ? isPointInSelectedBbox(mousePosition, selectedIds, sections)
  : false;
...
useEffect(() => {
  // ... 其他光标逻辑
    ...
    if (isBoxSelectingRef.current) {
      ...
      setCursorStyle('move');
    } else if (isInSelectedBbox) {
      // 鼠标在选中框范围内（但不在元素上），显示 grab 光标
      setCursorStyle('grab');
    } else {
      setCursorStyle('default');
    }
  }
}, [/* deps */, isInSelectedBbox]);

// 3. 修改鼠标按下处理，添加选中框内拖拽检测
const handleMouseDown = useCallback((e: React.MouseEvent) => {
  // ... 前面的逻辑
  ...
  // 如果没有点击到元素，检查是否在选中元素的边界框内
  if (selectedIds.size > 0 && isPointInSelectedBbox(worldPoint, selectedIds, sections)) {
    // 在选中框范围内开始拖拽
    isDraggingElementRef.current = true;
    setIsDraggingElement(true);
    dragStartPointRef.current = worldPoint;
...
**原有问题：**
- 元素可选中范围是元素顶点构成的矩形框（bounding box）
- 点击元素外的空白区域（但在矩形框内）会错误地选中元素
...
**验证结果：**
经代码审查，`findElementAtPoint` 函数已经使用 `isPointInPolygon` 进行精确多边形检测：
...
**原有问题：**
`findElementsInBox` 函数使用边界框相交检测（`doBoundingBoxesIntersect`），导致框选时只要元素的边界框与选择框相交就被选中，即使多边形实际并不相交。

#### C.4.2 解决方案

修改 `findElementsInBox`，在边界框相交的基础上，增加多边形与选择框的精确相交检测。
...
1. **快速排除**：先检查多边形边界框与选择框是否相交，不相交则直接返回 false
2. **多边形顶点检测**：检查多边形是否有顶点在选择框内
3. **选择框顶点检测**：检查选择框的四个角是否有多边形内
4. **边相交检测**：检查多边形的边是否与选择框的边相交

#### C.4.4 测试要点

- 框选范围与元素边界框相交但与多边形不相交时，**不应该**选中元素
- 框选范围与多边形实际相交时，**应该**选中元素
- 框选范围完全包含多边形时，**应该**选中元素

### C.5 文件修改清单

| 文件 | 修改内容 |
|------|----------|
| `app/src/utils/selection.ts` | 1. 新增 `isPointInBoundingBox` 函数<br>2. 新增 `getCombinedBoundingBox` 函数<br>3. 新增 `isPointInSelectedBbox` 函数 |
| `app/src/components/canvas/Canvas.tsx` | 1. 新增 `mousePosition` 状态<br>2. 新增 `isInSelectedBbox` 计算<br>3. 修改光标样式逻辑，在选中框范围内显示 grab<br>4. 修改 `handleMouseDown`，添加选中框内拖拽检测 |

### C.5 验证清单

- [x] 选中单个元素后，光标在选中框（蓝色虚线框）内任意位置显示 grab
- [x] 选中单个元素后，点击选中框内任意位置可拖拽元素
- [x] 选中多个元素后，光标在组合选中框内任意位置显示 grab
- [x] 选中多个元素后，点击组合选中框内任意位置可拖拽所有选中元素
- [x] 元素外、选中框内的区域可以正常拖拽（而不是取消选择）
- [x] 点击选中框外正常取消选择
- [x] 精确点击元素仍然正常工作
- [x] 按住 Shift/Ctrl 多选逻辑不受影响
- [x] 元素选中使用多边形精确检测而非矩形框
